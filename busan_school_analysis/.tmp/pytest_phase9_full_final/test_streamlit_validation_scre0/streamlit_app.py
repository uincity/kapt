...
if page == "Overview":
    ...
    school_district = frames["schools"][["school_id", "sigungu"]].drop_duplicates("school_id")
    feeder = feeder.merge(school_district, left_on="elementary_school_id", right_on="school_id", how="left")
    feeder["busan_feeder_rank"] = feeder.elementary_feeder_score.rank(method="min", ascending=False)
    feeder["sigungu_feeder_rank"] = feeder.groupby("sigungu").elementary_feeder_score.rank(method="min", ascending=False)
    feeder_districts = ["전체", *sorted(feeder.sigungu.dropna().unique())]
    feeder_district = st.sidebar.selectbox("진학권 순위 구·군", feeder_districts)
    if feeder_district != "전체":
        feeder = feeder[feeder.sigungu.eq(feeder_district)]
    cols = ["busan_feeder_rank", "sigungu_feeder_rank", "sigungu", "elementary_school_name", "elementary_feeder_score", "eligible_middle_count", "weighted_middle_score",
            "worst_middle_score", "feeder_exclusivity", "feeder_uncertainty", "assignment_data_quality"]
    st.dataframe(feeder[cols], hide_index=True,column_config={"busan_feeder_rank":"부산 순위","sigungu_feeder_rank":"구·군 내 순위","sigungu":"구·군",
      "elementary_school_name":"초등학교","elementary_feeder_score":"진학권 점수",
    ...
    event = st.dataframe(view[cols], hide_index=True, on_select="rerun", selection_mode="single-row",
                         column_config={"busan_school_rank":"부산 순위","complex_name":"단지명","sigungu":"구·군","legal_dong":"법정동","households":"세대수",
                          "elementary_school_names":"초등학교","school_zone_score":st.column_config.ProgressColumn("학군점수",format="%.2f%%",min_value=0,max_value=100),
